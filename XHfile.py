...
def main():
    ...
    with open('output.txt', 'w', encoding='utf-8') as file:
        ...
        while continue_flag.lower() == 'y':
            ...
            for item in content:
                result += str(item)
            que= ('请依据《国际中文教育中文水平等级标准》中的“四维基准”，并结合老师所强调的内容，从以下教学对话中自动识别并提取重点词语、语音、语法或话题知识点，尤其是其运用。让我们一步一步思考，按照以下内容进行：1.阅读理解: 仔细阅读教学对话，捕捉重点词汇和句型。2.关键词提取: 识别并记录重复或强调的词汇。3.语法识别: 注意任何特别指出的语法结构。4.语音标注: 留意发音、声调等语音方面的讨论。5.话题概括: 提炼对话中的主题内容。6.结果组织: 将知识点按照结构化格式（如JSON）呈现。比如：“那我们通常可以说把一瓶酒喝完”，输出{语法：{“把字句”：把一瓶酒喝完}}。以下是需要你识别的内容：') + result
